...
# --dangerously-skip-permissions: auto-approve everything -- without it opencode can block on a
# permission prompt, which would hang forever since stdin is closed (</dev/null). The whole
# point of this tool is fully unattended runs, so every provider must run in "full auto" mode
# (confirmed: this is intentional, not a default-on footgun -- see README's "Adding a provider").
